...
if ($function == 'jSelectProduct') {
    $document->addStyleSheet(Uri::root(true) . '/plugins/vmcustom/stockablecustomfields/assets/css/products.css');
    $document->addScript(Uri::root(true) . '/plugins/vmcustom/stockablecustomfields/assets/js/products.js');
    ...
    if (!empty($product_id) && !empty($custom_id)) {
        ...
        $parent_customfields = CustomfieldStockablecustomfield::getCustomfields($product_id, $custom_id, $limit = false, 'disabler', '=', 0);
	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search"><?php echo Text::_('JSEARCH_FILTER_LABEL'); ?>
			...
			<div class="filter-select fltrt btn-group pull-right">
				<select id="virtuemart_category_id" class="vm-chzn-select"
					...
					<option value="">
					<?php echo Text::sprintf( 'COM_VIRTUEMART_SELECT' ,  Text::_('COM_VIRTUEMART_CATEGORY')) ; ?>
					</option>
					<?php echo $this->category_tree; ?>
				</select>
				<?php echo HTMLHelper::_('select.genericlist', $this->manufacturers, 'virtuemart_manufacturer_id', 'class="vm-chzn-select" onchange="this.form.submit(); return false;"', 'value', 'text',
		...
		<table class="adminlist table table-striped">
			<thead>
				<tr>
					<th></th>
					<th class="title" width="20%"><?php echo HTMLHelper::_('grid.sort', 'COM_VIRTUEMART_PRODUCT_NAME', 'product_name', $listDirn, $listOrder); ?>
					</th>
					<th width="20%"><?php echo HTMLHelper::_('grid.sort',  'COM_VIRTUEMART_PRODUCT_CHILDREN_OF', 'product_parent_id', $listDirn, $listOrder); ?>
					</th>
					<th width="3%"><?php echo Text::_('COM_VIRTUEMART_PRODUCT_PARENT_LIST_CHILDREN'); ?>
					</th>
					<th width="12%"><?php echo HTMLHelper::_('grid.sort',  'COM_VIRTUEMART_PRODUCT_SKU', 'product_sku', $listDirn, $listOrder); ?>
					</th>
					<th width="8%"><?php echo HTMLHelper::_('grid.sort',  'COM_VIRTUEMART_PRODUCT_PRICE_TITLE', 'product_price', $listDirn, $listOrder); ?>
					</th>
					<th><?php echo Text::_('COM_VIRTUEMART_CATEGORY'); ?>
					</th>
					<th width="15%"><?php echo HTMLHelper::_('grid.sort', 'COM_VIRTUEMART_MANUFACTURER_S', 'mf_name', $listDirn, $listOrder); ?>
					</th>

					<th width="2%"><?php echo HTMLHelper::_('grid.sort', 'COM_VIRTUEMART_ID', 'p.virtuemart_product_id', $listDirn, $listOrder); ?>
					</th>
				</tr>
			</thead>
			<tfoot>
            ...
            foreach ($this->productlist as $key => $product) {
                if (isset($product->product_price_display)) {
                    ...
                    $price = Text::_('COM_VIRTUEMART_MULTIPLE_PRICES');
                } else {
                    $price = Text::_('COM_VIRTUEMART_NO_PRICE_SET');
                }
                ...
                $has_children_msg = !empty($child_prod) ? Text::_('JYES') : Text::_('JNO');
                ?>
