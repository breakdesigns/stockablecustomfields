...
if(version_compare(JVERSION, '3.0','<')){
	$document->addStyleSheet(Uri::root().'components/com_virtuemart/assets/css/chosen.css');
	$document->addScript(Uri::root().'components/com_virtuemart/assets/js/chosen.jquery.min.js');
	vmJsApi::chosenDropDowns();
}

if($function=='jSelectCustom'){
	$document->addStyleSheet( Uri::root(true).'/plugins/vmcustom/stockablecustomfields/assets/css/list.css');
	$document->addStyleSheet( Uri::root(true).'/plugins/vmcustom/stockablecustomfields/assets/css/mybootstrap.css');
	$document->addScript( Uri::root(true).'/plugins/vmcustom/stockablecustomfields/assets/js/list.js');
}
	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible cfhide"><?php echo Text::_('JSEARCH_FILTER_LABEL');?></label>
				<input type="text" name="keyword" id="filter_search" placeholder="<?php echo Text::_('JSEARCH_FILTER');?>" value="<?php echo $this->escape($keyword); ?>" />
			</div>

			<div class="btn-group pull-left">
				<button type="submit" class="btn hasTooltip">
				<?php echo version_compare(JVERSION, '3.0','ge')?'<i class="icon-search"></i>':Text::_('JSEARCH_FILTER_SUBMIT');?>
				</button>
				<button type="button" class="btn hasTooltip"
					onclick="document.id('filter_search').value='';this.form.submit();">
					<?php echo version_compare(JVERSION, '3.0','ge')?'<i class="icon-remove"></i>':Text::_('JSEARCH_FILTER_CLEAR');?>
				</button>
			</div>
		...
		<table class="adminlist table table-striped">
			...
			<tr>
				<th></th>
				<th width="20%"><?php echo Text::_('COM_VIRTUEMART_TITLE'); ?></th>
				<th width="10%"><?php echo Text::_('COM_VIRTUEMART_CUSTOM_FIELD_TYPE'); ?></th>
				<th width="10%"><?php echo Text::_('COM_VIRTUEMART_CUSTOM_GROUP'); ?></th>
				<th width="35%"><?php echo Text::_('COM_VIRTUEMART_CUSTOM_FIELD_DESCRIPTION'); ?></th>
				<th><?php echo Text::_('COM_VIRTUEMART_PUBLISHED'); ?></th>
				<th><?php echo Text::_('COM_VIRTUEMART_CUSTOM_IS_CART_ATTRIBUTE'); ?></th>
				...
				<tr class="row<?php echo $key % 2; ?>" id="element_<?php echo $custom->virtuemart_custom_id?>">
					...
					if($function=='jSelectCustom' && $compatible):?>
						<button type="button" class="breakdesigns_btn addBtn"
							title="<?php echo Text::_('PLG_STOCKABLECUSTOMFIELDS_ADD_CUSTOM_LABEL')?>"
							onclick="addToList('<?php echo Text::_($custom->custom_title);?>', '<?php echo Text::_($custom->field_type_display);?>', '<?php echo $custom->virtuemart_custom_id;?>');">
							<i class="bdicon-plus"></i>
						</button>
						<button type="button" class="breakdesigns_btn removeBtn"
							title="<?php echo Text::_('PLG_STOCKABLECUSTOMFIELDS_REMOVE_CUSTOM_LABEL')?>"
					...
					<td align="left">
						<?php
						echo Text::_($custom->custom_title);
						?>
					</td>
					<td align="right"><?php
						echo Text::_($custom->field_type_display);
						?>
					</td>
					<td align="left">
						<?php
						...
						$text='';
						if(!empty($custom->custom_parent_title))$text = $lang->hasKey($custom->custom_parent_title) ? Text::_($custom->custom_parent_title) : $custom->custom_parent_title;
						else if(!empty($custom->group_title))$text = $lang->hasKey($custom->group_title) ? Text::_($custom->group_title) : $custom->group_title;
						echo HTMLHelper::_('link', JRoute::_($link,FALSE),$text, array('title' => Text::_('COM_VIRTUEMART_FILTER_BY').' '.htmlentities($text)));
                        ?>
                    </td>
					<td align="left"><?php
						echo Text::_($custom->custom_desc);
						?>
					</td>
					<td align="right">
